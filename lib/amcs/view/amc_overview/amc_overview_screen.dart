...
class AmcOverviewScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => AmcOverviewCubit(
        repository: context.read<AmcRepository>(),
        trnRepository: context.read<TransactionRepository>(),
      ),
...
class _AmcOverviewScreenState extends State<_AmcOverviewScreen> {
  ...
  void initState() {
    super.initState();
    context.read<AmcOverviewCubit>().fetchAmcOverview();
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.amcId)),
      body: Padding(
                      ...
                      children: [
                        Row(
                          children: [
                            Text(
                              'IOCL',
                              style: TextStyle(color: Colors.teal.shade500, fontSize: 18, fontWeight: FontWeight.bold),
                            ),
