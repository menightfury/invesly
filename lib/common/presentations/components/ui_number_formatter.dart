...
class UINumberFormatter {
  ...
  bool get _shouldCompact => compactView == true && amount.abs() >= _compactLimit;

  String get _currencySymbolWithoutDecimalSep => currency!.symbol.replaceAll(currentDecimalSep, '');
  ...
  String _getFormattedAmount() {
    ...
    switch (mode) {
      ...
      case UINumberFormatterMode.percentage:
        formattedAmount = NumberFormat.decimalPercentPattern(decimalDigits: showDecimals ? 2 : 0).format(amount);
        break;
  ...
  String _getFormattedCurrencyAmount(String decimalSep) {
    if (_shouldCompact) {
      final formatter = NumberFormat.compactCurrency(decimalDigits: 3, symbol: _currencySymbolWithoutDecimalSep);
  ...
  String _getFormattedDecimalAmount() {
    if (_shouldCompact) {
      ...
      return NumberFormat.decimalPatternDigits(decimalDigits: showDecimals ? 2 : 0).format(amount);
    }
  }
  ...
  List<TextSpan> getTextSpanList(BuildContext context) {
    ...
    if (mode != UINumberFormatterMode.currency) {
      return _getTextSpanListForAFormattedNumber(formattedAmount, fontSize: valueFontSize);
    }
